...
public class Gestion {
    ...
    private void gestRestitution() {
        lloc.sort(Comparator.comparing(o -> o.getExemplaire().getMatricule()));
        int choix = choixListe(lloc);
        Location loc = lloc.get(choix - 1);
        loc.setDateRestitution(LocalDate.now());
        System.out.println("Exemplaire retourné");

        System.out.println("L'état de l'exemplaire a-t-il changé? (y/n)");
        String response = sc.next();
        if (response.equalsIgnoreCase("y")) {
            System.out.println("Entrez le nouvel état de l'exemplaire:");
            String etat = sc.next();
            loc.getExemplaire().setEtat(etat);
            System.out.println("État de l'exemplaire mis à jour");
        }
    }
    ...
    private void gestRayons() {
        ...
        while (true) {
            lex.sort(Comparator.comparing(o -> o.getOuvrage().getTitre()));
            int choix = choixListe(lex);
            Exemplaire ex = lex.get(choix - 1);
            if (ex.getRayon() != null) {
                System.out.println("Exemplaire déjà dans un rayon");
                continue;
            }
            r.addExemplaire(ex);
            System.out.println("Exemplaire ajouté au rayon");

            System.out.println("Voulez-vous ajouter un autre exemplaire? (y/n)");
            String response = sc.next();
            if (!response.equalsIgnoreCase("y")) {
                break;
            }
    ...
    private void gestExemplaires() {
        ...
        Collections.sort(lex, Comparator.comparing(Exemplaire::getMatricule));
    ...
    private void gestOuvrages() {
      /*  Ouvrage o = null;
        ...
        switch (choix){
                ...
                case 3 :
                            ...
                            do{
                                ...
                                ((DVD)o).getAutresLangues().add(langues.get(choix-1));  }while(true);
        ...
        while (true) {
            ...
            int choix = choixListe(laut);
            Auteur a = laut.get(choix - 1);
            if (o.getAuteurs().contains(a)) {
                System.out.println("Auteur déjà attribué à cet ouvrage");
                continue;
            }
            o.addAuteur(a);
            System.out.println("Auteur attribué à l'ouvrage");

            System.out.println("Voulez-vous attribuer un autre auteur? (y/n)");
            String response = sc.next();
            if (!response.equalsIgnoreCase("y")) {
                break;
            }
    ...
    private void gestAuteurs() {
        ...
        while (true) {
            ...
            if (a.getOuvrages().contains(o)) {
                System.out.println("Ouvrage déjà attribué à cet auteur");
                continue;
            }
            a.addOuvrage(o);
            System.out.println("Ouvrage attribué à l'auteur");

            System.out.println("Voulez-vous attribuer un autre ouvrage? (y/n)");
            String response = sc.next();
            if (!response.equalsIgnoreCase("y")) {
                break;
            }
